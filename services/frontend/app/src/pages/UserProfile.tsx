...
const UserProfile = () => {
  ...
  if (loading) return <div className="p-20 text-center font-bold text-gray-400">Loading user profile...</div>;
    ...
    <div className="max-w-2xl mx-auto py-12 px-6">
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 flex flex-col items-center">
        <img 
          ...
          alt="Avatar"
        />
        <h1 className="text-3xl font-black text-gray-900 capitalize">{profile.firstName} {profile.lastName}</h1>
        <p className="text-blue-600 font-bold mb-2 uppercase tracking-widest text-xs">{profile.jobTitle || 'Member'}</p>
        <p className="text-gray-500 text-center max-w-md mb-8 leading-relaxed italic">
          "{profile.bio || "No bio added yet."}"
        </p>
        
        <div className="w-full border-t border-gray-50 pt-6 flex justify-around text-center">
          <div>
            <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-1">Email Address</p>
            <p className="text-sm font-bold text-gray-700">{profile.email || 'Private'}</p>
          </div>
          <div>
            <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-1">Member Since</p>
            <p className="text-sm font-bold text-gray-700">{new Date(profile.createdAt).toLocaleDateString()}</p>
          </div>
        </div>
      </div>
    </div>
